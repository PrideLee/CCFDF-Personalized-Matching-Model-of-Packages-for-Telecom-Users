...
if __name__ == '__main__':
    ...
    def forest_class(file_dataframe, paralist):
        ...
        for min_sample_split in min_samples_split_list:
            ...
            RFModel = ensemble.RandomForestClassifier(n_estimators=iTrees, max_depth=depth, max_features=maxFeat, n_jobs=-1,
                                                      min_samples_split=min_sample_split, oob_score=False, random_state=531)
        ...
        plot.plot(min_samples_split_list, F1_list)
        ...
        plot.show()
        return error_id, best_confusionnMat
    ...
    id_error, mat_con = forest_class(balance_data, para_list)
    F1 = F1_score(mat_con)
    print(mat_con)
    print(F1)
    # id_error_dataframe = pd.DataFrame({"error_id": id_error})
    # best_id_error_dataframe = pd.DataFrame({"best_error_id": best_id_error})
    # id_error_dataframe.to_csv(r"E:\CCFDF\plansmatching\result\class_2\RF\error_id_balance_correct_1.csv",
    #                           encoding="utf-8")
    # best_id_error_dataframe.to_csv(r"E:\CCFDF\plansmatching\result\class_2\RF\best_error_id_balance_correct_1.csv",
    #                                encoding="utf-8")
    # joblib.dump(rf_best, r"E:\CCFDF\plansmatching\result\class_2\RF\best_rf_5_balance_correct_1.pkl")
