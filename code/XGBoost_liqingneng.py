...
num_test = len(test_8)
test_0_8 = [i for i in range(num_test) if (test_8[i] == 0 or test_8[i] == 7)]
data_0_8 = data_test.iloc[test_0_8]
model_0_8 = joblib.load(r'E:\CCFDF\plansmatching\data\raw data\final_data\Chanle_B\xgb_0_8_up_real.model')
binary_0_8 = model_0_8.predict(data_0_8)
num_0_8 = len(test_0_8)
for i in range(num_0_8):
    test_8[test_0_8[i]] = binary_0_8[i]
print(test_8)
print('Correction by the two class classifier')
print("Accuracy : %.2f" % accuracy_score(label_test, test_8))
confusion_mat = confusion_matrix(label_test, test_8)
print("Test confusion matrix")
print(confusion_mat)
...
submit_num = len(submit_label_encode)
submit_0_8 = [i for i in range(submit_num) if (submit_label_encode[i] == 0 or submit_label_encode[i] == 7)]
submit_data_0_8 = data_submit.iloc[submit_0_8]
submit_binary_0_8 = model_0_8.predict(submit_data_0_8)
submit_num_0_8 = len(submit_0_8)
for i in range(submit_num_0_8):
    submit_label_encode[submit_0_8[i]] = submit_binary_0_8[i]
...
final_da.to_csv(r"E:\CCFDF\plansmatching\data\raw data\final_data\Chanle_B\xgb_0_8_up_real.csv", index=False)
