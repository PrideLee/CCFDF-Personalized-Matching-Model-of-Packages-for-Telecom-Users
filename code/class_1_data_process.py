...
def fee(par, file_dataframe):
    ...
    fee_mini = fee_matrix.min(axis=0)
    fee_dis = (fee_max - fee_mini).tolist()
...
train_data_all = pd.read_csv(r"E:\CCFDF\plansmatching\data\raw data\final_data\train_all\train_all\train_all.csv",
                              encoding="utf-8", low_memory=False)

test_data_all = pd.read_csv(
    r"E:\CCFDF\plansmatching\data\raw data\final_data\republish_test\republish_test_pro\repiulish_test_duplicate.csv",
                        encoding="utf-8", low_memory=False)

train_type = train_data_all['service_type'].tolist()
train_len = len(train_type)
index_train_1 = [i for i in range(train_len) if train_type[i] == 1]
train_1 = train_data_all.iloc[index_train_1]

train_1 = type_decode(train_1)


test_type = test_data_all['service_type'].tolist()
test_len = len(test_type)
index_test_1 = [i for i in range(test_len) if test_type[i] == 1]
test_1 = test_data_all.iloc[index_test_1]


num_train = len(train_1)
train_1_current_service = train_1['current_service'].tolist()
train_1_service_type_encode = train_1["service_type_encode"].tolist()
train_pop_id = train_1.drop(["current_service", "service_type_encode"], axis=1)
combine_data = pd.concat([train_pop_id, test_1])
...
fee_pay = pay_fee(pay)
fee_min = min_fee(fee_pay, ['1_total_fee', '2_total_fee', '3_total_fee', '4_total_fee'])
month_trf = traffic_current_month(fee_min)
exp_fee = fee_exp(['1_total_fee', '2_total_fee', '3_total_fee', '4_total_fee'], month_trf)
fee_pare = fee_par(exp_fee, ['online_time', 'month_traffic', 'contract_time', 'last_month_traffic', 'local_trafffic_month',
...
para_list = ["online_time", "fee_mean", "fee_std", "fee_fluctuate", "fee_distance", "month_traffic", "contract_time",
             "pay_num", "pay_mean", "pay_fee_subtract", "pay_total", "pay_fee_total_subtract", "fee_min",
             ...
             "service_caller_time_fluctuate", "service_caller_time_mean", "traffic_current_month",
             "1_total_fee", "2_total_fee", "3_total_fee", "4_total_fee", "fee_mat_subtract_01", "fee_mat_subtract_12",
             "fee_mat_subtract_23", "fee_mat_subtract_0112",
             "fee_mat_subtract_1223", "fee_mat_sum_01", "fee_mat_sum_12", "fee_mat_sum_23", "fee_mat_sum_0112",
             "fee_mat_sum_0112", "online_time_per_fee", "month_traffic_per_fee", "contract_time_per_fee",
             "last_month_traffic_per_fee", "local_trafffic_month_per_fee", "local_caller_time_per_fee",
             "service_caller_time_mean_per_fee"]
final_data = Min_Max(para_list, fee_pare)
train_data_pro = final_data.iloc[0:num_train]
test_data_pro = final_data.iloc[num_train:len(final_data)]
train_data_pro["current_service"] = train_1_current_service
train_data_pro["service_type_encode"] = train_1_service_type_encode

train_data_pro.to_csv(r"E:\CCFDF\plansmatching\data\raw data\final_data\train1_final.csv", index=False)
test_data_pro.to_csv(r"E:\CCFDF\plansmatching\data\raw data\final_data\test1_final.csv", index=False)
